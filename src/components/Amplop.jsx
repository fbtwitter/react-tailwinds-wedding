...
const Amplop = ({ openInvitation, setOpenInvitation }) => {
  ...
  return (
    <>
      {/* <div
        className={`absolute top-0 bottom-0 right-0 left-0 translate-y-0 z-[1000] ${
          openInvitation
            ? ''
            : 'translate-y-[-100vh] transition-all ease-in-out delay-75'
        }`}
      > */}
      <main
        className={`absolute top-0 bottom-0 right-0 left-0 transform transition-all translate-y-0  z-[100] duration-1000 ${
          openInvitation ? '' : 'translate-y-[-100vh]'
        }`}
      >
        <div className="relative w-screen h-screen flex flex-col justify-center items-center bg-[url('../assets/png/bg-green.png')] bg-cover">
          <img
            src={LeafLeft}
            alt="top-left"
